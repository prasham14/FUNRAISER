...
import ImprovedLoader from './Spinner';
...
const MyFunds = () => {
  ...
  const handleDeleteClick = async (fundId) => {
    ...
    try {
      await axios.delete(`https://funraiser-pvio.vercel.app/fund/deleteFund/${fundId}`, {
        headers: {
  ...
  const handleDeleteDoc = async (fundId) => {
    ...
    try {
      await axios.delete(`https://funraiser-pvio.vercel.app/fund/deleteDoc/${fundId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  useEffect(() => {
    const fetchFunds = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(`https://funraiser-pvio.vercel.app/fund/getFunds/${userId}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  const handleEditSubmit = async () => {
    try {
      await axios.put(`https://funraiser-pvio.vercel.app/fund/editFund/${editInitiativeId}`, editData, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
